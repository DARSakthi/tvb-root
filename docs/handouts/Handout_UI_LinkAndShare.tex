...
\item In \underline{Settings} change the admin email.
\item Create a user for your neighbour
\end{enumerate}
\end{formal}
...
\item Create a second project (e.g. \textit{Session\_II\_ShareAndLink\_a}). 
\item Assuming that you are working in the first project, upload a Connectivity in a zip file. This was already done but you can repeat these steps. 
\item Go to \textsc{Projects}$\rightarrow$ \textsc{Data structure}. Click on \underline{Update Data}. An overlay with the current supported formats will appear. See Fig. XXX. 
\end{enumerate}
\end{formal} 
...
\begin{formal}
\begin{enumerate}
\item Select the connectivity you want to share. The metadata overlay will appear. You'll see a list with all your projects. See Fig. XXXX.
\item Go to the tab \underline{Links}. Select the second project.
\end{enumerate}
\end{formal} 

(Snaphsot With the link tab here)
