...
the model parameters (Session \#4). We choose epileptic regions in the right limbic areas 
...
\begin{simulation}
  ...
  \item Choose an $\mathbf{integration\:step\:size}$ of $\mathbf{0.1}$, a $\mathbf{raw\:monitor}$ and $\mathbf{\unit{6000}[ms]}$ as a simulation length. 
...
\subsection{Surface based simulation of an epileptic patient}
 ...
 It also allows to have a more accurate representation of the MEG/EEG/sEEG signal.
  ...
  \item Choose the TVB $\mathbf{default\:surface}$, the $\mathbf{corresponding\:local\:connectivity}$ and a $\mathbf{Local\:coupling\:strength}$ of $\mathbf{a=0.2}$.
...
For more documentation on The Virtual Brain, please see the following articles \citep{Sanz-Leon_2013, Woodman_2014, Jirsa_2014}
