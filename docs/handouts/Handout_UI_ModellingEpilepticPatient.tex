...
forward solution of MEG  with interictal spikes, modelisation of surgery with resection of a part of the brain.\sidenote{
...
i.e. you will be able to do region and surface simulation.} 
...
\begin{margintable}
  ...
  \begin{tabular}{l}
    ...
    Modeling surgical resection\\
     \indent \textit{Surface\_Resection} \\
    \bottomrule
  \end{tabular}
...
\begin{formal}
  ...
  is the center of a limit cycle. Both states are separated by a separatrix, as you can se by trying different trajectories
  in this phase space (left click on the figure) limit cycle.
  \item Now change the value of the parameter $\mathbf{x_0}$ to the value $\mathbf{-2.2}$ and see what happens to the phase space: the cubic nullcline
  is raised such that only the leftmost fix point now exists. The model is set in a non-epileptogenic state.
  \item You can also look at the phase space for other variables, such as $y_2$/$y_0$ (slow-fast subsystem) or $y_5$/$y_4$ (second population)
  \end{enumerate}
\end{formal}
...
\newthought{We are going} to model a patient with temporal lobe epilepsy (TLE)	. For this we choose different values of
epileptogenicity in the Epileptor $x_0$ parameters according to the region positions, this is a case of heterogeneity 
(Session \#4). We choose epileptic regions in the right limbic areas 
...
\begin{simulation}
  ...
  \item Configure the \underline{Visualizers} and add a \underline{Brain Viewer} and a \underline{Fourier Spectrum}, 
  then \underline{Save your choices}.
  \item Choose an integration $\mathbf{step size}$ of $\mathbf{0.1}$, a $\mathbf{raw monitor}$ and $\mathbf{\unit{6000}[ms]}$ as a simulation length. 
...
\begin{simulation}
  ...
  you want to add.
  \item For this simulation, we will not use the \underline{Brain Viewer} so reconfigure the Viewer Tabs accordingly.
\end{enumerate}
\end{simulation}
...
\begin{simulation}
  ...
  \item Go back, click on the sEEG and visualize them with the 3d/2d visualizer (Fig. \ref{fig:EEG}).
\end{enumerate}
\end{simulation}
...
\subsection{Surface based simulation of an epileptic patient}
 ...
 \newthought{To account} also for seizure propagation and not only seizure recruitement, we have to use surface based simulations.
 It also allow to have a more accurate representation of the MEG/EEG/sEEG signal.
 
  \begin{simulation}
  \begin{enumerate}
  \item Copy the former simulation.
  \item Add a \underline{Brain Viewer} visualizer.
  \item Choose the TVB default surface, the corresponding local connectivity and a $\mathbf{Local\:coupling\:strength}$ of $\mathbf{a=0.2}$.
  \item Add a \underline{Temporal average} monitor.
  ...
  Then click on \underline{Submit Surface parameters}. You can see that the field of the $x_0$ parameter is updated with
...
\end{figure}
 ...
 \begin{simulation}
  ...
  move the brain.
  \item Go back,  click on TimeSeries and visualize them with the spatial average time series.
  with the \underline{Time Series Visualizer}. Change the scaling, change the selected channels and zoom in to see
  the seizure (Fig. \ref{fig:ts_surf}).
  \item Go back, click on the sEEG time series and visualize them with the 3d/2d visualizer (Fig. \ref{fig:surf_sEEG}).
...
\subsection{Modeling a stimulus}
...
The stimulus was already set for you under the name \textit{Surface\_SquareStimulus}
  ...
  \item Set the $\mathbf{Simulation\:Length}$ to \unit{4000}[ms].
...
\end{marginfigure}
  ...
  the Animated Time Series visualizer.
  \item \underline{Select channel} \#28 to \#36 (Fig. \ref{fig:choose_channels}) increase the page size in Brain Menu to include time \unit{2000}[ms] 
...
\newthought{Often} we do not have access with MEG to an epileptic seizure, but only to interictal periods.
...
\begin{simulation}
  ...
  \item Copy the surface simulation \textit{Surface temporal lobe EEG MEG sEEG} and paste the array of values in the parameter $\mathbf{x_0}$.
  Change the values $\mathbf{-1.6}$ by $\mathbf{-2.2}$ in this array (i.e. we replace the dyanmics of the resected node by a stable node).
