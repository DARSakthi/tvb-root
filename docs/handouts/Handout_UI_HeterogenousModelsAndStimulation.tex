...
define the \underline{spatial profile of the stimulus}. For the spatial profile we'll use e \underline{Gaussian function} with the only change being the \underline{width of the kernel}, that we set to \textbf{\unit[10]{mm}}.

\item We also need to specify one or
more \underline{focal points} (vertices on the cortical surface), about which the
\underline{spatial equation} will be evaluated. To do this we go to \underline{View Stimulus progress} where we can select a few focal points. 
\item Here, as with the \underline{local connectivity}, we
